...
// 				    initInputModal();
		...
		$(".orderMethod").click(function(){
			...
			if(isSearch == true){
				let searchName = $("#findEmpName").val().toLowerCase();
				$.ajax({
				    url: "orderFindEmpByName.do", // 데이터가 송수신될 서버의 주소
				    type: "GET", // 통신 방식 (GET, POST, PUT, DELETE)
					data : {"searchName" : searchName, "orderMethod" : orderMethod}, // 데이터 보내기
				    dataType: "json", // 수신 받을 데이터 타입 (MIME TYPE)
				    success: function (data) {
					    console.log(data);
					    if(data.status == "success"){
...
<!-- 		정렬방식 선택 -->
  ...
  <input type="radio" class="form-check-input orderMethod" id="radio1" name="optradio" value="employee_id" checked>사번순 (오름차순)
  <label class="form-check-label" for="radio1"></label>
</div>
<div class="form-check">
  <input type="radio" class="form-check-input orderMethod" id="radio2" name="optradio" value="hire_date">입사일순 (내림차순)
